...
class DeletePostUseCaseTest {
    ...
    fun setUp() {
        ...
        useCase = DeletePostUseCase(repositories)
    }

    @Test
    fun `invoke should call deletePost on correct repository`() =
        runTest {
            coEvery { subscriptionsRepository.deletePost(feedPost) } just Runs

            useCase(feedPost, NewsFeedType.SUBSCRIPTIONS)

            coVerify { subscriptionsRepository.deletePost(feedPost) }
        }
