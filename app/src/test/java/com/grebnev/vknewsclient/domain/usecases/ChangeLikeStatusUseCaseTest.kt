...
class ChangeLikeStatusUseCaseTest {
    @MockK
    ...
    private lateinit var useCase: ChangeLikeStatusUseCase

    @Before
    fun setUp() {
        MockKAnnotations.init(this)
        val repositories = mapOf(NewsFeedType.SUBSCRIPTIONS to subscriptionsRepository)
        useCase = ChangeLikeStatusUseCase(repositories)
    }

    @Test
    fun `invoke should call changeLikeStatus on correct repository`() =
        runTest {
            coEvery { subscriptionsRepository.changeLikeStatus(feedPost) } just Runs

            useCase(feedPost, NewsFeedType.SUBSCRIPTIONS)

            coVerify { subscriptionsRepository.changeLikeStatus(feedPost) }
        }
